...
def main():
    parser = argparse.ArgumentParser(description="Unit-style validation for qwen v3.1 Gemma bridge")
    parser.add_argument(
        "--config",
        type=str,
        default="config/libero_train_qwen_3_5_2b_gemma_bridge_v3_1_smoke1000_filmcond_textsummary.yaml",
    )
    ...
    _assert(
        str(cfg.model.architecture) == "qwen_3.5_2b_gemma_bridge_v3_1",
        "Expected v3.1 bridge architecture config",
    )
    _assert(
    ...
    _assert(prefix_memory["tap_indices"] == [full_attention_indices[-1]], "tap_strategy=last must choose last full-attention layer")
    ...
    _assert(prefix_memory["memory"].shape[-1] == int(cfg.action_head.vlm_hidden_dim), "Tap memory width must remain Qwen hidden size before bridge")
    _assert("text_attention_mask" in prefix_memory, "Prefix memory must expose text_attention_mask")
    _assert(
        prefix_memory["text_attention_mask"] is not None
        and prefix_memory["text_attention_mask"].shape == prefix_memory["attention_mask"].shape,
        "text_attention_mask must align with attention_mask",
    )
    _assert(
        bool(prefix_memory["text_attention_mask"].any().item()),
        "text_attention_mask unexpectedly contains no attended text tokens",
    )
    _assert("instruction_summary" in prefix_memory, "Prefix memory must expose instruction_summary")
    _assert(
        prefix_memory["instruction_summary"] is not None
        and prefix_memory["instruction_summary"].shape == (1, int(cfg.action_head.vlm_hidden_dim)),
        "instruction_summary must have shape [B, hidden_dim]",
    )
    _assert(
    ...
    _assert("memory_norm_ratio" in outputs, "v3.1 bridge forward must report memory_norm_ratio")
    ratio = float(outputs["memory_norm_ratio"].detach().cpu().item())
    _assert(np.isfinite(ratio), "memory_norm_ratio is not finite")
    _assert(
        ratio <= float(cfg.action_head.get("memory_norm_ratio_limit", 8.0)),
        f"memory_norm_ratio exceeded configured limit: {ratio}",
    )
    ...
    tmp_dir = Path(tempfile.mkdtemp(prefix="qwen35_v31_bridge_"))
    try:
        torch.save(
            {
        ...
        _assert(not missing and not unexpected, f"Round-trip checkpoint mismatch: missing={missing} unexpected={unexpected}")
    ...
    print("v31_bridge_unit_tests_ok", True)
